...
- (id)initWithFrame:(CGRect)frame
{
	...
	self.barWidth			= 20.0;
	self.barAlpha			= 1.0;
	self.chartBorderColor	= [UIColor grayColor];
	self.gridColor			= [UIColor grayColor];
	self.hasYLabels			= YES;
	self.yLabelFont			= [UIFont fontWithName:@"Din Alternate" size:12.0];
	self.yLabelColor		= [UIColor grayColor];
	self.xLabelFont			= [UIFont fontWithName:@"Din Alternate" size:12.0];
	self.xLabelColor		= [UIColor grayColor];
	self.xLabelType			= SimpleBarChartXLabelTypeVerticle;
	self.barTextFont		= [UIFont fontWithName:@"Din Alternate" size:12.0];
...
- (void)reloadData
{
	if (_dataSource)
	{
        ...
        CGSize yLabelSize		= self.hasYLabels ? [[NSString stringWithFormat:@"%li", (long)_topValue] sizeWithAttributes:@{NSFontAttributeName:self.yLabelFont}] : CGSizeZero;
...
- (void)setupYAxisLabels
{
    ...
    CGSize yLabelSize			= [[NSString stringWithFormat:@"%li", (long)_topValue] sizeWithAttributes:@{NSFontAttributeName:self.yLabelFont}];
...
- (void)setupBarTexts
{
	...
	for (NSInteger i = 0; i < _numberOfBars; i++)
	{
        ...
        if (barLabelText.length > 1) {
            barText.text			= [NSString stringWithFormat:@"%.2f", [barLabelText floatValue]];
        }
        else { barText.text			= @""; }
